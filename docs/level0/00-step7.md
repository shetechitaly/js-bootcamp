...
_7.2 – the new `filterItems()` function_
...
But there’s a catch, `updateList()` will also save our tasks each time we call it! We want this to happen when we _modify_ our task list \(for example adding, deleting, and marking items as done\), but we don’t want this if we are simply applying a filter.
...
_7.3 – the updated `updateList()` function_
...
_7.4 – the new `clearCompleted()` function_
